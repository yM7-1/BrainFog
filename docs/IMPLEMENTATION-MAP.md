...
9. **图例与画线**：地图图例保持可见（按 0.02 第 9 条"图例可见"）；画线工具、右键绘制与快捷键自 0.3.3 起保持原版（不再隐藏/拦截）。
