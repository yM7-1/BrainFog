...
> 验证状态：`bash tools/check.sh` = Release 构建 + 45 单测 + 补丁目标审计全绿。**游戏内行为待人工验收（见下）。**
...
6. **玩家视野"周围一圈"**（0.01 4.1 的可见半径遮罩）尚未实现（需要后处理/着色器），当前仅敌人遮蔽。
...
10. **战斗**：敌人=呼吸方框；敌人第 1 回合有意图，之后无；受击特效/伤害数字仍在；己方召唤物正常可见。
