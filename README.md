...
- 状态：**Phase 1–3 功能已实现，0.3.x 扩展六项认知规则 + 遗物全场景显示 + 记忆消逝删卡动画，0.3.2 起默认值调整**（见 `CHANGELOG.md`；Release 构建 + 全部单测 + 补丁目标审计全绿，数量以 `tools/check.sh` 输出为准），游戏内行为待人工验收（见 `docs/IMPLEMENTATION-MAP.md` §3）
