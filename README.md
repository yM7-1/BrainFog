...
生命与金币默认实时却同样是乱码（可切换「失忆模式」停留在"上次休息时"的记忆里，或开启「血量数/金币数恢复正常显示」让数值可读）；敌人、遗物与关底 Boss 只剩轮廓和问号（可切换「敌人模型可见」）。
**认知修改器在主标题界面与对局内均可操作**（文字 / 认知 / 感知三模块，可重置为默认）；开启「卡牌计数器」后右上角出现出牌计数排行榜（同名副本固定编号，可收起）。
你仍然能操作，只是认不出任何东西。

- 状态：**Phase 1–3 功能已实现，0.3.0 新增六项认知规则**（初始卡组揭示 / 敌人模型可见 / 新入组卡牌揭示与 n=2 / 记忆消逝 / 失忆提醒 / 卡牌计数器；Release 构建 + 全部单测 + 补丁目标审计全绿，数量以 `tools/check.sh` 输出为准），游戏内行为待人工验收（见 `docs/IMPLEMENTATION-MAP.md` §3）
