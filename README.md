...
卡牌要打过一次才能认出来（「好记性」）或打两次也认不出（「坏记性」）；所有文字（界面、事件、对话、菜单，含主标题）按统一比例碎成乱码（面板可调 0%–100%，乱码可固定或每次重进随机）；
生命与金币默认实时却同样是乱码（可切换「失忆模式」停留在"上次休息时"的记忆里，或开启「血量数/金币数恢复正常显示」让数值可读）；敌人、遗物与关底 Boss 只剩轮廓和问号。
**认知修改器在主标题界面与对局内均可操作**（文字 / 认知 / 感知三模块，可重置为默认）。
你仍然能操作，只是认不出任何东西。
