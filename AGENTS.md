...
## 项目当前状态（2026-09-25，v0.3.9）

- 目标游戏 **STS2 v0.107.1 – v0.111.0**（正式服 + 测试服共用同一 DLL）+ **RitsuLib 0.6.2**；仅单人（联机自动禁用）
- **测试**：`tools/check.sh` 全绿（installed 249 项；min 版本编译 + 0.107.1/0.111.0 双版本审计 251 项）
- **代码/文档**：GitHub `main` 同步至本批次；工作区干净
- **工坊**：线上版本 **0.3.9**（条目 3804204184，2026-09-25 更新：关闭开关 + 迭代批次 + 卡牌文字修复；描述与 changenote 已同步）；本地 `mods/BrainFog` 已清除，自测请用订阅版
- **功能现状**（0.3.5 已实机验收；0.3.6–0.3.9 待实机确认）：
  ...
  - 0.3.8：Neow 加血补间断修复（实时模式不再显示 0/最大HP）；地图图例开关往返不再卡住可读状态；
    用户开关与联机自动禁用共用恢复/重应用路径，重开时强制重乱码关闭期间新建文本；
    新增版本号/本地化/补丁声明反向校验测试与 CardRevealTracker/SelectionRevealPlanner 边界测试；
    BRAINFOG_DEBUG dump 与 F9 覆盖层扩展
  - 0.3.9：卡牌文字乱码恢复修复（重开开关后对无原文记录的在场卡牌补扫，牌面文字立即按比例乱码）
...
- **版本历史**：`CHANGELOG.md`（0.3.9 / 0.3.8 / 0.3.7 / 0.3.6 / 0.3.5 / 0.3.4 / 0.3.3 / 0.3.2 / 0.3.1 / 0.3.0 …）
