...
- **感知**：血量/金币失忆模式；血量数/金币数恢复正常显示；显示已拥有遗物；显示地图所有路线；敌人模型可见；敌人意图（不可见 / 仅第一回合 / 全部可见）
...
开局牌组视为记得（好记性/坏记性）。**记忆消逝**（开关，默认开）：战斗结束时仍未揭示的牌会从卡组中移除（歪比巴卜模式不受影响，无保底）。**卡牌计数器**（开关，默认关）：记录每张牌（同名按加入顺序编号，如「打击1」）本局打出的次数，右上角显示可收起的排行榜。
...
- **世界模糊**：敌人只剩呼吸方框、名字隐藏（可开启「敌人模型可见」）；遗物不可见、药水只剩轮廓、地图迷雾、Boss 信息隐藏；受击/治疗数字乱码
...
- **Perception**: HP/gold amnesia mode; readable HP/gold numbers; show owned relics; show all map routes; visible enemy models; enemy intents (hidden / first round only / always)
...
The starting deck counts as known (good/bad memory). **Memory fade** (on by default): cards still unrevealed at the end of a combat are removed from the deck (nonsense mode exempt; no floor). **Play counter** (off by default): counts plays per copy (same-name copies numbered by joining order, e.g. "Strike1") and shows a collapsible top-right leaderboard.
...
- **Blurred world**: enemies are breathing boxes with hidden names (visible enemy models can be enabled); relics invisible, potions outlined, map fogged, boss info hidden; damage/heal numbers garbled
...
感§：血量/金▓▓忆模&；血量▒/金#数恢&正※▓◇；◇示已拥有◆◆；显示&图◆有路░；敌人模@可※；◇▒▒#（░可见 / 仅@#※合 / 全部◆@）
◇▒＊#@模#
◆▒%物：%＊◇牌§@可见
◇░░：打＊/&◇%▒即＊久%◇，§名＊§所#奖▒/商店/事▒中◆#示真▓面
坏%性（▒＊）：◆#%&▒ n 次※打出%会忘◇（n 默认 2、可调）；§▓◇@§数；#◆░◆※◆认得，即※失忆※手牌%◇&提醒
歪#巴卜：@＊§%揭▓
※%@组▓为#得（好&▒/坏#◆）。记░%◆（※&，#认开）：§斗结▓※#＊░示░◇＊%卡组中移▒（歪比巴◇模※＊受影&，░保&）。░牌计◇器（§%，默░%）：记录每◆§（同§＊加#%序＊▒，如「@%1」）%▒▓出的次░，#上%&※可&起※&＊▓。
※※▒果
卡◆失认：开＊＊▓#▒记@；%░打过#牌&认得；奖励与◆牌界◇默认&#示稀＊度
░▒失▓：事件、@单、░示、▓面&＊▓◇部※码（图◇◇设置※外），@码程%可%
记忆失真：#◆◆#币＊认实＊＊同样◆%码；◆切◆░▒模#停◆在「上次※息时」，或＊◆「血量※/金币@恢░&%@示」§数值可◇
世界%糊：敌人＊剩呼吸※#、名&＊░（@◆启「@人@型◆见」）；◇物◆&见、§水只剩◆※、▒图迷雾、Boss ◆░隐░；§击/治疗数※@＊
依赖 STS2 v0.111.0 与 RitsuLib 0.6.2；◆▓人；§#◆恢&§░。
Github&◇：https://github.com/yM7-1/BrainFog
